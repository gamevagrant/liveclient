...
import com.meetvr.share.request.LiveStopRequest;
...
public class LiveStopControl {
    ...
    private void hideProgress(){
        ...
        public LiveStopControl(final Context context, UserInfo userInfo, String his_id, final ProgessDlg progessDlg){
            this.context = context;
            this.progessDlg = progessDlg;
            try {
                if(userInfo!=null){
                    LiveStopRequest liveStopRequest = new LiveStopRequest(userInfo.getToken(),userInfo.getId());
                    liveStopRequest.addParam(his_id);
                    liveStopRequest.addQuery(context, new Request.OnNetWorkProc() {
                        @Override
                        public void onStart() {
                            if(progessDlg!=null)
